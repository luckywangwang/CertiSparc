...
| Add : forall M M' (R : RegFile) oexp (rs rd : GenReg) v1 v2,
    M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 ->
    indom (R rd) M -> MemMap.set (R rd) (Some (v1 +ᵢ v2)) M = M' ->
    R__ (M, R) (add rs oexp rd) (M', R)
        
| Sub : forall M M' (R : RegFile) oexp (rs rd : GenReg) v1 v2,
    M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 ->
    indom (R rd) M -> MemMap.set (R rd) (Some (v1 -ᵢ v2)) M = M' ->
    R__ (M, R) (sub rs oexp rd) (M', R)

| Subcc : forall M M' (R : RegFile) oexp (rs rd : GenReg) v1 v2 v,
    M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 ->
...
| And : forall M M' (R : RegFile) oexp (rs rd : GenReg) v1 v2,
    M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 ->
    indom (R rd) M -> MemMap.set (R rd) (Some (v1 &ᵢ v2)) M = M' ->
    R__ (M, R) (and rs oexp rd) (M', R)

| Andcc : forall M M' (R : RegFile) oexp (rs rd : GenReg) v1 v2 v,
    M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 ->
    indom (R rd) M -> indom (R n) M -> indom (R z) M -> v = v1 &ᵢ v2 ->
    set_Rs R M ((Rr rd, v) :: (Rpsr n, get_range 31 31 v) :: (Rpsr z, iszero v) :: nil) = M' ->
    R__ (M, R) (andcc rs oexp rd) (M', R)

| Rd : forall M M' (R : RegFile) (rsp : SpReg) (ri : GenReg) v,
...
| SSave :
    forall (M M' M'' : Memory) (R : RegFile) D F F' k oexp fmo fml fmi fm1 fm2 v1 v2 (rs rd : GenReg),
      M (R cwp) = Some k -> fetch M R = Some [fmo; fml; fmi] ->
      M (R rs) = Some v1 -> eval_opexp R M oexp = Some v2 -> F = F' ++ (fm1 :: fm2 :: nil) ->
      M' = set_window R M fm1 fm2 fmo -> 
      M'' = set_Rs R M' ((Rr rd, v1 +ᵢ v2) :: (Rpsr cwp, pre_cwp k) :: nil) ->
